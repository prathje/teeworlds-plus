...
@interface AccountServerView : NSTextView
{
	NSTask *task;
	NSFileHandle *file;
}
...
@implementation AccountServerView
- (void)listenTo: (NSTask*)t;
{
	NSPipe *pipe;
	task = t;
	...
	[[NSNotificationCenter defaultCenter] addObaccountserver: self selector: @selector(outputNotification:) name: NSFileHandleReadCompletionNotification object: file];
...
void runAccountServer()
{
	...
	[window setTitle: @"Teeworlds AccountServer"];

	view = [[[AccountServerView alloc] initWithFrame: graphicsRect] autorelease];
	[view setEditable: NO];
	...
	[view listenTo: task];
	[task setLaunchPath: [mainBundle pathForAuxiliaryExecutable: @"accountsrv"]];
